...
def guess_carried_scalar_type(col) -> type:
    ...
    if isinstance(col, data_algebra.pandas_model.default_data_model.pd.core.series.Series):
        col = col.values
    if len(col) < 1:
        return type(None)
    good_idx = numpy.where(
        numpy.logical_not(data_algebra.pandas_model.default_data_model.pd.isna(col))
    )[0]
    test_idx = 0
    if len(good_idx) > 0:
        test_idx = good_idx[0]
...
def guess_column_types(
    ...
    for c in columns:
        ...
        if (
            ...
            or gt == data_algebra.pandas_model.default_data_model.pd.core.series.Series
        ):
