...
class OpC(data_algebra.data_ops_types.OperatorPlatform):
    ...
    def to_near_sql_implementation(self, db_model, *, using, temp_id_source):
        return self.nd.to_near_sql_implementation(
    ...
    def project_parsed(self, parsed_ops=None, *, group_by=None):
        self.set(self.nd.project_parsed(parsed_ops=parsed_ops, group_by=group_by))
        return self

    def project(self, ops=None, *, group_by=None):
        self.set(self.nd.project(ops=ops, group_by=group_by,))
        return self

    def natural_join(self, b, *, by, jointype):
        self.set(self.nd.natural_join(b=b, by=by, jointype=jointype))
        return self

    def concat_rows(self, b, *, id_column="source_name", a_name="a", b_name="b"):
        self.set(
            self.nd.concat_rows(b=b, id_column=id_column, a_name=a_name, b_name=b_name)
        )
        return self

    def select_rows_parsed(self, parsed_expr):
        self.set(self.nd.select_rows_parsed(parsed_expr=parsed_expr))
        return self

    def select_rows(self, expr):
        self.set(self.nd.select_rows(expr=expr,))
        return self

    def drop_columns(self, column_deletions):
        self.set(self.nd.drop_columns(column_deletions=column_deletions))
        return self

    def select_columns(self, columns):
        self.set(self.nd.select_columns(columns=columns))
        return self

    def rename_columns(self, column_remapping):
        self.set(self.nd.rename_columns(column_remapping=column_remapping))
        return self

    def order_rows(self, columns, *, reverse=None, limit=None):
        self.set(self.nd.order_rows(columns=columns, reverse=reverse, limit=limit))
        return self
