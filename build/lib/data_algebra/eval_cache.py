...
def hash_data_frame(d) -> str:
    ...
    """
    data_algebra.data_model.data_model_type_map["default_data_model"].is_appropriate_data_instance(d)
    hash_str = hashlib.sha256(
        data_algebra.data_model.data_model_type_map["default_data_model"].pd.util.hash_pandas_object(d).values
    ).hexdigest()
...
def make_cache_key(
    *,
    ...
    for k in data_map_keys:
        ...
        assert data_algebra.data_model.data_model_type_map["default_data_model"].is_appropriate_data_instance(data_map[k])
    return EvalKey(
