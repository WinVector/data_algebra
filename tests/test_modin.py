...
def test_modin():
    ...
    data_model = None
    try:
        data_model = data_algebra.modin_model.ModinModel(
            modin_engine=modin_engine_choice
        )
    ...
    if (modin_pandas is not None) and (data_model is not None):
        ...
        ops = describe_table(d, table_name="d").extend({"y": "2 * x"})
        ...
        expect_start = data_algebra.default_data_model.pd.DataFrame({"x": [1, 2],})
...
def test_modin_gcalc():
    ...
    if (modin_pandas is not None) and (data_model is not None):
        d = modin_pandas.DataFrame({"x": [1, 2, 3, 5], "g": ["a", "a", "b", "b"]})

        ops = describe_table(d, table_name="d").extend(
            {"x_mean": "x.mean()"}, partition_by=["g"]
        )
