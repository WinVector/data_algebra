...
def test_ex_examples_fn_notation_1_cant_ex_short_descr():
    d = data_algebra.pandas_model.default_data_model.pd.DataFrame({"x": range(20), "z": 7,})
...
def test_ex_examples_fn_notation_1_short_descr():
    ...
    res = ops.transform(d)
    expect = d = data_algebra.pandas_model.default_data_model.pd.DataFrame({"x": range(20)})
    assert data_algebra.test_util.equivalent_frames(expect, res)
