...
def test_simple_expr_1():
    d_orig = data_algebra.pandas_model.default_data_model.pd.DataFrame({"x": [1.0, 2.0, -3.0, 4.0]})
    d = d_orig.copy()
    ...
    expect = data_algebra.pandas_model.default_data_model.pd.DataFrame({"x": [1.0, 2.0, -3.0, 4.0]})
    expect["z"] = expect["x"] + 1
