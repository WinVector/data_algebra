...
def test_rename_columns_1():
    ...
    expect = data_algebra.default_data_model.pd.DataFrame({"x2": [1], "y1": [2], "z": ["q"]})
    assert data_algebra.test_util.equivalent_frames(res_pandas, expect)
...
def test_map_columns_del():
    ...
    res_pandas = ops.transform(d)
    expect = data_algebra.default_data_model.pd.DataFrame({"y1": [2], "z": ["q"]})
    assert data_algebra.test_util.equivalent_frames(res_pandas, expect)
    data_algebra.test_util.check_transform(ops, data={"d": d}, expect=expect)


def test_map_columns_swap_del():
    d = data_algebra.default_data_model.pd.DataFrame({"x": [1], "y": [2], "z": [3]})
    td = describe_table(d, table_name="d")
