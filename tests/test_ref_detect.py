...
def test_ref_detect_1():
    ...
    ops = describe_table(d, table_name='d'). \
        rename_columns({'x_coded_left': 'x_coded',
                        'idx_left': 'idx'}). \
        natural_join(b=describe_table(d, table_name='d'),
                     jointype='full',
                     by=['x']). \
    ...
    with pytest.raises(Exception):
        ops = describe_table(d, table_name='d'). \
            rename_columns({'x_coded_left': 'x_coded',
                            'idx_left': 'idx'}). \
            natural_join(b=describe_table(d2, table_name='d'),
                         jointype='full',
                         by=['x']). \
        select_rows('x_coded_left < x_coded')
...
def test_ref_detect_2():
    ...
    ops = describe_table(d, table_name='d'). \
        rename_columns({'x_coded_left': 'x_coded',
                        'idx_left': 'idx'}). \
        natural_join(b=describe_table(d, table_name='d'). \
                     rename_columns({'x_coded_right': 'x_coded',
                                     'idx_right': 'idx'}),
                     jointype='full',
                     by=['x']). \
