...
def test_set_quoting_flex():
    d = data_algebra.data_model.data_model_type_map["default_data_model"].pd.DataFrame({"x": ["a", None]})
    ops = describe_table(d, table_name="d").extend({"select": "x.is_in({'a'})"})
    ops.transform(d)


def test_set_quoting_mismatch():
    d = data_algebra.data_model.data_model_type_map["default_data_model"].pd.DataFrame({"x": ["a", "b"]})
