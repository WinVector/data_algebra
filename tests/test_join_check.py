...
def test_join_check():
    d1 = data_algebra.data_model.data_model_type_map["default_data_model"].pd.DataFrame({"key": ["a", "b"], "x": [1, 2],})
    td1 = describe_table(d1, table_name="d1")

    d2 = data_algebra.data_model.data_model_type_map["default_data_model"].pd.DataFrame({"key": ["b", "c"], "y": [3, 4],})
