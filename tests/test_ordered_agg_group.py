...
def test_ordered_agg_group():
    ...
    diagram = data_algebra.data_model.default_data_model().pd.DataFrame(
        {
            "rank": [1, 2, 3],
    ...
    ops = (
        ...
        .extend({"rank": "_row_number()"}, partition_by=["ID"], order_by=["DATE", "OP"])
    ...
    res_db = data_algebra.data_model.default_data_model().pd.read_sql_query(sql, con)
    ...
    expect_ag = data_algebra.data_model.default_data_model().pd.DataFrame(
        {
            "ID": [1, 2, 3, 4, 5, 6],
            "DATE1": [
                "2001-01-02 00:00:00",
