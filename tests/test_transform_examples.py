...
def test_extend_transform_1():
    d = data_algebra.pandas_model.default_data_model.pd.DataFrame({"x": [1, 2]})
    ops = describe_table(d, table_name="d").extend({"y": 1})
    expect = data_algebra.pandas_model.default_data_model.pd.DataFrame({"x": [1, 2], "y": [1, 1],})
...
def test_extend_transform_2():
    ...
    expect = data_algebra.pandas_model.default_data_model.pd.DataFrame({"x": [1, 2], "y": [2, 4],})
    data_algebra.test_util.check_transform(ops, d, expect)
...
def test_natural_join_transform_1():
    d1 = data_algebra.pandas_model.default_data_model.pd.DataFrame({"k": ["a", "b"], "x": [1, 2],})
...
def test_concat_rows_transform_1():
    d1 = data_algebra.pandas_model.default_data_model.pd.DataFrame({"k": ["a", "b"], "x": [1, 2],})
    d2 = data_algebra.pandas_model.default_data_model.pd.DataFrame({"k": ["b", "c"], "x": [3, 4],})
...
def test_select_rows_transform_1():
    d = data_algebra.pandas_model.default_data_model.pd.DataFrame({"x": [1, 2], "y": [3, 4]})
    ops = describe_table(d, table_name="d").select_rows("x == 2")
    expect = data_algebra.pandas_model.default_data_model.pd.DataFrame({"x": [2], "y": [4]})
    data_algebra.test_util.check_transform(ops, d, expect)
...
def test_drop_columns_transform_1():
    ...
    expect = data_algebra.pandas_model.default_data_model.pd.DataFrame({"y": [3, 4]})
    data_algebra.test_util.check_transform(ops, d, expect)
...
def test_rename_columns_transform_1():
    ...
    expect = data_algebra.pandas_model.default_data_model.pd.DataFrame({"y": [1, 2], "x": [3, 4]})
    data_algebra.test_util.check_transform(ops, d, expect)
...
def test_order_rows_transform_1():
    ...
    expect = data_algebra.pandas_model.default_data_model.pd.DataFrame({"x": [2, 1], "y": [4, 3]})
    data_algebra.test_util.check_transform(ops, d, expect, check_row_order=True)


def test_convert_records_transform_1():
    iris_small = data_algebra.pandas_model.default_data_model.pd.DataFrame(
        {
