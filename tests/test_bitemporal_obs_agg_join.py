...
def test_join_where_merge():
    d_observations = data_algebra.pandas_model.default_data_model.pd.DataFrame({
        "target_date": [10, 10, 11, 11],
        "as_of_date": [8, 9, 9, 10],
        })
    d_actions = data_algebra.pandas_model.default_data_model.pd.DataFrame({
    ...
    with sqlite3.connect(":memory:") as conn:
        ...
        res = data_algebra.pandas_model.default_data_model.pd.read_sql_query(sql_query, conn)
    ...
    with sqlite3.connect(":memory:") as conn:
        d_observations.to_sql("d_observations", conn, index=False)
        d_actions.to_sql("d_actions", conn, index=False)
        res_sql2 = data_algebra.pandas_model.default_data_model.pd.read_sql_query(ops.to_sql(data_algebra.SQLite.SQLiteModel()), conn)
